      ...
      {posts.map((post, index) => {
        return (
          <Link
            to={`/post/${post._id}`}
            key={post._id}
            className=" dark:bg-primary bg-white drop-shadow rounded overflow-hidden"
          >
            <div className=" min-w-[300px] h-[300px] w-full relative cursor-pointer">
              {post.images[0]?.url.endsWith("mp4") ? (
                <video
